...
const githubCard = document.querySelector(".github__card");
const toggleThemeMode = document.querySelector(".theme__mode--wrapper");
const error = document.querySelector(".error");
const ottocatProfile = "octocat";
...
let storedTheme =
  ...
  (window.matchMedia("(prefers-color-scheme: dark)").matches
    ...
    : "light");
if (storedTheme)
  document.documentElement.setAttribute("color-mode", storedTheme);

// resource: https://lukelowrey.com/css-variable-theme-switcher/
toggleThemeMode.onclick = function() {
  let currentTheme = document.documentElement.getAttribute("color-mode");
...
// render GitHub card and get GitHub API data
async function getUserProfile(username) {
  ...
  async function getURL() {
    ...
    const getDate = new Date(data.created_at).toUTCString().slice(5, 16); 
   
    githubCard.innerHTML = `
        <div class="github__card--profile">
        <div class="avatar"><img src="${data.avatar_url}" alt="profile picture of GitHub user"></div>
        <div class="profile__header">
          <h2 class="fs-heading fs-name">${data.name}</h2>
          <h3 class="username">@${data.login}</h3>
          <p class="joined">Joined  ${getDate}</p>
        </div>
        <div class="bio">
          <p>
            ${
      ...
      <ul class="github__card--stats">
        <li class="stats--wrapper stats-repos--wrapper">
          <h3 class="fs-stats-text stats-repos">Repos</h3>
          <span class="fs-heading fs-stats-totals stats-repos--total">${
            data.public_repos
          }</span>
        </li>
        <li class="stats--wrapper stats-followers--wrapper">
          <h3 class="fs-stats-text stats-followers">Followers</h3>
          <span class="fs-heading fs-stats-totals stats-followers--total">
            ${data.followers}
          </span>
        </li>
        <li class="stats--wrapper stats-following--wrapper">
          <h3 class="fs-stats-text stats-following">Following</h3>
          <span class="fs-heading fs-stats-totals stats-following--total">${
            data.following
          }</span>
        </li>
      </ul>

      <ul class="github__card--social flow">
        <li class="social--wrapper location  ${(data.location  === '' || data.location === null)  ? "not-available"  : ''}" ${(data.location  === '' || data.location === null)  ? 'class=not-available'  : ''}">
        ...
        <li class="social--wrapper twitter  ${(data.twitter_username  === '' || data.twitter_username === null)  ? "not-available"  : ''}" ${(data.twitter_username  === '' || data.twitter_username === null)  ? 'class=not-available'  : ''}">
        ...
        <li class="social--wrapper website ${(data.blog  === '' || data.blog === null)  ? "not-available"  : ''}" ${(data.blog  === '' || data.blog === null)  ? 'class=not-available'  : ''}">
        ...
        <li class="social--wrapper company ${(data.company  === '' || data.company === null)  ? "not-available"  : ''}" ${(data.company  === '' || data.company === null)  ? 'class=not-available'  : ''}">
...
// allow user to search for GitHub user via username
searchUserBtn.addEventListener("click", (e) => {
  const username = searchUserInput.value;
  // let r = requests.get(url.format(username)).json();
  if (username) {
    getUserProfile(username);
    username.value = "";
    error.classList.remove("error--not-hidden");
  } else if (username === "") {
    error.classList.add("error--not-hidden");
  }
});
